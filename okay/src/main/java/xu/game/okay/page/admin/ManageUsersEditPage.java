...
public class ManageUsersEditPage extends JPanel {
    ...
    public ManageUsersEditPage(String username) {
        ...
        password_field.setText(Jdbc.query("select password from biao1 where name = " + "'" + username + "'"));
        jComboBox.setSelectedItem(Integer.valueOf(Jdbc.query("select guan from biao1 where name = " + "'" + username + "'")));
    ...
    public void mouse(JButton ok, JButton nok, JButton menu, JTextField username_field, JTextField password_field, JComboBox jComboBox) {
        ok.addActionListener(e -> {
            String string = Jdbc.query("select name from biao1 where name = '" + username_field.getText() + "';");
            if (username_field.getText().equals(""))
                ...
                String password = Jdbc.query("select password from biao1 where name = '"
                        + username + "'");
                String guan = Jdbc.query("select guan from biao1 where name = '"
                        + username + "'");
                if (list.size() > 0) {
                    Iterator iterator = list.iterator();
                    while (iterator.hasNext()) {
                        Jdbc.update("delete from biao3 where nguan = " + iterator.next()
                                + " AND name = '" + username + "';");
                    }
                }
                list.removeAll(list);
                if (!username_field.getText().equals(username)) {
                    Jdbc.update("update biao1 set name = '" + username_field.getText() +
                            "' where name = '" + username + "'");
                    Jdbc.update("update biao3 set name = '" + username_field.getText() +
                            "' where name = '" + username + "'");
                }
                if (!password_field.getText().equals(password)) {
                    Jdbc.update("update biao1 set password = '" + password_field.getText() +
                            "' where name = '" + username + "'");
                }
                if (!jComboBox.getSelectedItem().equals(guan)) {
                    Jdbc.update("update biao1 set guan = '" + jComboBox.getSelectedItem() +
                            "' where name = '" + username + "'");
                }
