package xu.game.okay.page.users;
...
public class GametimePage extends JPanel {
    ...
    public void caozuo() {
        ...
        if (guan == 0) {      //自定义模式
            Jdbc.update("INSERT INTO biao4 (tuxing,shuxing,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10) " +
        ...
        string = Jdbc.query("SELECT count(*) FROM biao4;");
        ...
        if (pd) {
            ...
            if (ball_x < -10 || ball_y < -10 || ball_x > 800 || ball_y > 1000) {
                ...
                if (i == number) {               //清除完
                    ...
                    timer = null;
                    String guanSQL;
                    guanSQL = Jdbc.query("select guan from biao1 where name = '" + username + "';");
                    Jdbc.update("delete from biao4 where number > 0;");
                    Jdbc.update("ALTER TABLE biao4 AUTO_INCREMENT= 1;");
                    ...
                    if (Main.admin == false) {
                        if (guan > 0) {
                            if (guan < 20 && guan == Integer.valueOf(guanSQL))
                                Jdbc.update("update biao1 set guan = " + ++guan + " where name ='" + username + "';");
                            else guan++;
                        }
                        if (guan == 0) {              //自定义模式
                            Main.user_defined_menu();
    ...
    public void fantan(Graphics2D g) {
        ...
        if (dangtu[1] == 1) {               //多边形
            ...
            zhi_x = Jdbc.query("select x1 from biao4 where number = " + (dangtu[0] + 1) + ";");
            zhi_y = Jdbc.query("select y1 from biao4 where number = " + (dangtu[0] + 1) + ";");
