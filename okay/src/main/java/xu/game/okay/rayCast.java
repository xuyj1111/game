    ...
    public boolean rayCasting(boolean yes_no) {
        ...
        if (flag == true && yes_no == true && gametimePage.fan_tan_x > 0 && gametimePage.moo == 6) {
            ...
            for (tuxnumb = 1; tuxnumb <= 20; tuxnumb++) {
                String zhi;
                zhi = Jdbc.query("select count(*) from biao4 where number = " +
            ...
            for (int i = 0; i < tuxnumb; i++) {
                String zhix, zhiy;
                double zhi;
                zhix = Jdbc.query("select " + "x" + String.valueOf(i + 1) + " from biao4 where number = " +
                        (number + 1) + ";");
                zhiy = Jdbc.query("select " + "y" + String.valueOf(i + 1) + " from biao4 where number = " +
                        (number + 1) + ";");
            ...
            gametimePage.fan_tan_xx = Integer.valueOf(Jdbc.query("select " + "x" + String.valueOf(whitch + 1) + " from biao4 where number = " +
                    String.valueOf(number + 1) + ";")) + 5;
            gametimePage.fan_tan_yy = Integer.valueOf(Jdbc.query("select " + "y" + String.valueOf(whitch + 1) + " from biao4 where number = " +
                    String.valueOf(number + 1) + ";")) + 5;
        }
