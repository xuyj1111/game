    ...
    public void mouse(JTextField username, JPasswordField password, JPasswordField password_second,
        ...
        ok.addActionListener(e -> {
            ...
            if (username.getText().equals(""))
                ...
                string = Jdbc.query("select name from biao1 where name = '" + username.getText() + "';");
                if (username.getText().equals(string))
                    ...
                    if (password.getText().equals(password_second.getText())) {              //返回login_panel
                        int number = Integer.valueOf(Jdbc.query("select count(*) from biao1 where name != 'admin'"));
                        if (number == 18)
                            JOptionPane.showMessageDialog(null, "用户已满，不能注册 ");
                        else {
                            Jdbc.update("INSERT INTO biao1 VALUES ('" + username.getText() + "','" +
                                    password.getText() + "',1);");
                            Main.login();
                        }
                    } else
